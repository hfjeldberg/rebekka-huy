...
if($this->edit_mode) {
	...
	if(!function_exists('slider')) {
		...
		function slider($content, $values) {
			...
			if($images) { 
				$output .= '<div class="slider-wrapper ' . $pagination_position . '">';
				$output .= '<div class="is-gallery"></div>';
				$output .= '<div class="gallery-preview"><img src="' . $preview[0] . '" /></div>';
				$output .= '[cegallery id="slider-' . str_replace('#', '', $values['id']) . '" data_timeout="' . $values['options']['timeout'] . '" data_autoplay="' . $values['options']['autoplay'] . '" images="' . $content . '" nav="' . $values['options']['nav'] . '" nav_color="' . $values['options']['nav-color'] . '" pagination="' . $values['options']['pagination'] . '" pagination_color="' . $values['options']['pagination-color'] . '"][/cegallery]';
				$output .= '</div>';
			}
