...
if($edit_mode === 'edit' || $edit_mode === 'new' || $edit_mode === 'single-edit') {
	...
	if($content_type === 'multi-column') {
		...
		// add column
		$editor->add_column(str_replace('#', '', $editor->id), str_replace('#', '', $editor->parent_id), 'span6', $editor->column_count);
	} else {
		// load standard edit
	...
	for( $i = 1; $i <= $size; $i++) {
		...
		if($offset_arr['length'][$i] > 12) {
			...
			if(get_field('use_cyrillic', 'options') === 'enabled' && !version_compare(phpversion(), '5.4.0', '<')) {
