<?php
	...
	if(!$is_fwt) {
		// holder start
		$e .= '<div id="masonry-' . $options['masonryid'] . '-holder">';
	}
	...
	if ( $query->have_posts() ) {	
		while ( $query->have_posts() ) {
			...
			if($is_fwt) {
				...
				$e .= '<div ' . $margin_fix . ' class="thumb grid-item masonry-thumbs-item masonry-' . $options['masonryid'] . '-item masonry-' . $options['masonryid'] . '-item-' . $index . ' ' . $thumb_class . get_field('thumbnail_width') . '" data-thumb-src="' . $thumbnail[0] . '">';
				...
				if($thumbnail) {
					$e .= '<img alt="' . get_the_title() . '" src="' . $thumbnail[0] . '" width="500" height="500" />';
				} else {
					$e .= '<img alt="' . get_the_title() . '" height="500" src="' . get_bloginfo('template_directory') . '/images/no_thumb.png" />';
				}
				$e .= '</div>';
				if(!$hide_title) {
	...
	if(!$is_fwt) {
		...
		// get grid
		$e .= semplice_grid($options['masonryid'], '', $is_fluid, $remove_gutter, $index, $pre);
